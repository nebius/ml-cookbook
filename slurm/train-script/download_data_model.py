...
def download_model(model_name, model_dir, hf_token=None, force=False):
    ...
    if force:
        ...
        if os.path.isdir(model_dir) and any(os.scandir(model_dir)):
            print(
                f"[skip] Model directory already populated at {model_dir}; use --force_download to refresh."
            )
            return
        os.makedirs(model_dir, exist_ok=True)
    print(f"Downloading model {model_name} to {model_dir}")
    snapshot_dir = snapshot_download(  # nosec B615
        repo_id=model_name,
        repo_type="model",
        local_dir=model_dir,
        token=hf_token,
...
def download_dataset(dataset_name, data_dir, hf_token=None, force=False):
    print(
        f"Downloading and saving dataset {dataset_name} to {data_dir} using datasets.load_dataset/save_to_disk"
    )
    load_args = {}
    if hf_token:
    ...
    if force:
        ...
        if os.path.isdir(data_dir) and any(os.scandir(data_dir)):
            print(
                f"[skip] Dataset directory already populated at {data_dir}; use --force_download to refresh."
            )
            return
        os.makedirs(data_dir, exist_ok=True)
    dataset = load_dataset(dataset_name, **load_args)  # nosec B615
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Model repo name (e.g., org/model_name)"
    )
    parser.add_argument(
        "--dataset", type=str, required=True, help="Dataset repo name (e.g., org/dataset_name)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to store/download the model (default: <shared_folder>/model/<model_name>)",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default=None,
        help="Directory to store/download the dataset (default: <shared_folder>/data/<dataset_name>)",
    )
    parser.add_argument(
        "--force_download",
        action="store_true",
        help="Force re-download of model and dataset (clears existing)",
    )
