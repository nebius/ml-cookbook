...
def train(config):
    ...
    if not (0 <= local_rank < max(1, visible_gpu_count)):
        ...
        raise RuntimeError(
            f"local_rank {local_rank} is out of range for visible GPUs ({visible_gpu_count})"
        )
    ...
    if "train" not in dataset:
        raise RuntimeError(
            f"Dataset at {data_dir} does not contain a 'train' split. Found: {list(dataset.keys())}"
        )
    ...
    if val_split is None and not config.get("allow_no_validation", False):
        raise RuntimeError(
            f"No validation split found in dataset and 'allow_no_validation' not set. Splits: {list(dataset.keys())}"
        )

    # Load tokenizer
    tokenizer = hf.AutoTokenizer.from_pretrained(model_dir)  # nosec B615  (local dir, not a Hub fetch)

    # Infer task: explicit config['task'] preferred, otherwise use model config if available
    task = config.get("task", None)
    try:
        model_cfg = hf.AutoConfig.from_pretrained(model_dir)  # nosec B615  (local dir, not a Hub fetch)
        if task is None:
    ...
    if text_column is None:
        raise RuntimeError(
            f"Could not auto-detect a text input column in dataset columns: {train_cols}; set 'text_column' in config."
        )
    ...
    if task == "seq2seq":
        ...
        if target_column is None:
            raise RuntimeError(
                "Could not detect target text column for seq2seq task; set 'target_column' in config."
            )
    else:
    ...
    if task == "seq2seq":
        ...
        def tokenize_fn(examples):
            return tokenizer(
                examples[text_column], text_target=examples[target_column], truncation=True
            )

        token_presence_key = "input_ids"  # noqa: S105   (field name, not a secret)
    else:
    ...
    if not already_tokenized:
        if global_rank == 0:
            ...
            tokenized = dataset.map(
                tokenize_fn, batched=True, remove_columns=list(dict.fromkeys(remove_cols))
            )
    ...
    if global_rank == 0:
        try:
            ...
            for k, v in sample.items():
                print(
                    f"  {k}: {type(v)} -> {v if (isinstance(v, (int, float)) or (isinstance(v, list) and len(v) <= 5)) else type(v)}"
                )
            ...
            if bad:
                print(
                    "Found unexpected string values in tokenized fields (first 10 examples):", bad
                )
                raise RuntimeError(
                    "Tokenization produced string values in token fields; check tokenizer usage and dataset columns."
                )
    ...
    # -----------------------------
    if task == "seq2seq":
        model = hf.AutoModelForSeq2SeqLM.from_pretrained(model_dir)  # nosec B615  (local dir, not a Hub fetch)
    else:
        num_labels = int(config.get("num_labels", 2))
        model = hf.AutoModelForSequenceClassification.from_pretrained(  # nosec B615  (local dir, not a Hub fetch)
    ...
    forward_arg_names = (
        set(signature(model.module.forward).parameters.keys())
        if hasattr(model, "module")
        else set(signature(model.forward).parameters.keys())
    )
    ...
    train_sampler = DistributedSampler(
        dataset[train_split], num_replicas=world_size, rank=global_rank, shuffle=True
    )
    if val_split:
        val_sampler = DistributedSampler(
            dataset[val_split], num_replicas=world_size, rank=global_rank, shuffle=False
        )
    else:
        val_sampler = None
    ...
    if len(train_loader) == 0:
        raise RuntimeError(
            "Training dataloader is empty. Check dataset size, batch_size, and sampler configuration."
        )
    if val_loader is None:
        if not config.get("allow_no_validation", False):
            raise RuntimeError(
                "Validation dataloader not found. Set 'allow_no_validation' in config to proceed without validation."
            )
    else:
        if len(val_loader) == 0:
            raise RuntimeError(
                "Validation dataloader is empty. Provide a validation split or adjust dataset preparation."
            )
    ...
    if dist.is_initialized():
        ...
        obj = {
            "final_model_dir": config.get("final_model_dir"),
    ...
    for epoch in range(start_epoch, config["num_epochs"]):
        ...
        for step, batch in enumerate(
            tqdm(train_loader, desc=f"Epoch {epoch + 1} [Train]", disable=global_rank != 0)
        ):
            ...
            if task == "seq2seq":
                ...
                inputs = {
                    ...
                    if k not in ("labels", "label", "target_ids") and k in forward_arg_names
                }
            else:
                labels = None
                ...
                inputs = {
                    k: v
                    for k, v in batch.items()
                    if k not in ("labels", "label") and k in forward_arg_names
                }
        ...
        # compute averages
        train_loss = (
            (local_loss_sum.item() / local_sample_count.item())
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for batch in tqdm(
                    val_loader, desc=f"Epoch {epoch + 1} [Val]", disable=global_rank != 0
                ):
                    batch = {k: (v.to(device) if hasattr(v, "to") else v) for k, v in batch.items()}
                    if task == "seq2seq":
                        labels = None
            ...
            val_loss = (
                (local_val_loss_sum.item() / local_val_sample_count.item())
                if local_val_sample_count.item() > 0
                else 0.0
            )
    ...
    if global_rank == 0:
        ...
        if os.path.exists(checkpoint_path):
            if not config.get("keep_last_checkpoint", False):
                ...
                print(
                    f"[Rank 0] Deleted checkpoint file at {checkpoint_path} after successful training."
                )
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise TypeError(f"Config type error: {e}") from e
